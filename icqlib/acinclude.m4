...
## -*- shell-script -*-
## kicq.m4.in - macros for autoconf
##
## (c) 1999 Denis V. Dmitrienko <denis@null.net>
##
## http://www.cn.ua/~denis/kde/kicq.html
## http://www.cn.ua/~denis/kde/icqlib.html
##
dnl ------------------------------------------------------------------------
dnl ICQLIB_PATH([minimum_version])
dnl Try to find the icqlib headers and libraries.
dnl $(icq_libraries) will be the icqlib library location (if needed)
dnl $(icq_includes) will be the icqlib header location (if needed)
dnl $(ICQ_LDFLAGS) will be the -L$(icq_libraries) (if needed)
dnl $(ICQ_INCLUDES) will be the -I$(icq_includes) (if needed)
dnl ------------------------------------------------------------------------
dnl
AC_DEFUN(ICQLIB_PATH,
[
NEED_ICQLIB=$1
icq_major_version=`echo $NEED_ICQLIB | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\1/'`
icq_minor_version=`echo $NEED_ICQLIB | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\2/'`
icq_micro_version=`echo $NEED_ICQLIB | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\3/'`
ac_icq_includes=NO ac_icq_libraries=NO
AC_MSG_CHECKING([for icqlib >= $NEED_ICQLIB])
icq_libraries=""
icq_includes=""
AC_ARG_WITH(icq-includes,
    [  --with-icq-includes=DIR where the icqlib includes are.],
    [  ac_icq_includes="$withval" ])
    
AC_ARG_WITH(icq-libs,
    [  --with-icq-libs=DIR     where the icqlib library is installed.],
    [  ac_icq_libraries="$withval" ])

if test "$ac_icq_includes" = NO || test "$ac_icq_libraries" = NO; then

AC_CACHE_VAL(ac_cv_have_icq,
[#try to guess icqlib locations

icq_incdirs="/usr/local/include /usr/include /usr/lib/icqlib/include /usr/local/icqlib/include /usr/include/icqlib /usr/local/lib/icqlib/include"
AC_FIND_FILE(icq.h, $icq_incdirs, icq_incdir)
ac_icq_includes=$icq_incdir

icq_libdirs="/usr/local/lib /usr/lib /usr/lib/icqlib/lib /usr/local/icqlib/lib /usr/lib/icqlib"
AC_FIND_FILE(libicq.so libicq.so.$icq_major_version libicq.a, $icq_libdirs, icq_libdir)
ac_icq_libraries=$icq_libdir

ac_cxxflags_safe=$CXXFLAGS
ac_ldflags_safe=$LDFLAGS
ac_libs_safe=$LIBS
CXXFLAGS="$CXXFLAGS -I$icq_incdir"
LDFLAGS="$LDFLAGS -L$icq_libdir"
LIBS="-licq $LIBSOCKET"
...
#if !defined(ICQLIBMAJOR) || !defined(ICQLIBMINOR) || !defined(ICQLIBMICRO)
#error "You need icqlib $NEED_ICQLIB or greater. http://www.cn.ua/~denis/kde/icqlib.html"
#endif

#if !((ICQLIBMAJOR>$icq_major_version) || (ICQLIBMAJOR==$icq_major_version && ICQLIBMINOR>$icq_minor_version) || (ICQLIBMAJOR==$icq_major_version && ICQLIBMINOR==$icq_minor_version && ICQLIBMICRO>=$icq_micro_version))
#error "You need icqlib $NEED_ICQLIB or greater. http://www.cn.ua/~denis/kde/icqlib.html"
#endif
],
[
...
if test "$ac_icq_includes" = NO || test "$ac_icq_libraries" = NO; then
  ac_cv_have_icq="have_icq=no"
  ac_icq_notfound=""
  if test "$ac_icq_includes" = NO; then
    if test "$ac_icq_libraries" = NO; then
      ac_icq_notfound="(headers and libraries)";
    else
      ac_icq_notfound="(headers)";
    fi
  else
    ac_icq_notfound="(libraries)";
  fi

  AC_MSG_ERROR([icqlib >= $NEED_ICQLIB $ac_icq_notfound not found. Please check your installation! ]);
else
  have_icq="yes"
fi
])
...
if test "$have_icq" != yes; then
  AC_MSG_RESULT([$have_icq]);
else
  ac_cv_have_icq="have_icq=yes \
    ac_icq_includes=$ac_icq_includes ac_icq_libraries=$ac_icq_libraries"
  AC_MSG_RESULT([libraries $ac_icq_libraries, headers $ac_icq_includes])
  
  icq_libraries=$ac_icq_libraries
  icq_includes=$ac_icq_includes
fi
AC_SUBST(icq_libraries)
AC_SUBST(icq_includes)

if test "$icq_includes" = "$qt_includes"; then
 ICQ_INCLUDES="";
else
 ICQ_INCLUDES="-I$icq_includes"
 all_includes="$ICQ_INCLUDES $all_includes"
fi

if test "$icq_libraries" = "$qt_libraries"; then
 ICQ_LDFLAGS=""
else
 ICQ_LDFLAGS="-L$icq_libraries"
 all_libraries="$ICQ_LDFLAGS $all_libraries"
fi

AC_SUBST(ICQ_INCLUDES)
AC_SUBST(ICQ_LDFLAGS)
])

dnl ------------------------------------------------------------------------
dnl ICQLIB_VERSION()
dnl Generates ICQLIBVER, ICQLIBMAJOR, ICQLIBMINOR and ICQLIBMICRO constants
dnl ------------------------------------------------------------------------
dnl
AC_DEFUN(ICQLIB_VERSION,
[
ICQLIBV=`cat VERSION`
ICQLIBMAJOR=`echo $ICQLIBV | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\1/'`
ICQLIBMINOR=`echo $ICQLIBV | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\2/'`
ICQLIBMICRO=`echo $ICQLIBV | sed 's/\([[0-9]]*\).\([[0-9]]*\).\([[0-9]]*\)/\3/'`
ICQLIBVER=`printf "0x%02i%02i%02i" $ICQLIBMAJOR $ICQLIBMINOR $ICQLIBMICRO`
AC_SUBST(ICQLIBMAJOR)
AC_SUBST(ICQLIBMINOR)
AC_SUBST(ICQLIBMICRO)
AC_SUBST(ICQLIBVER)
])

dnl ------------------------------------------------------------------------
dnl ICQLIB_OPTBSWAP()
dnl Enables/disables optimized byteswap functions
dnl ------------------------------------------------------------------------
dnl
AC_DEFUN(ICQLIB_OPTBSWAP,
[
AC_ARG_ENABLE(optbswap,[  --disable-optbswap      disable optimized byteswap functions],
[ 
  ac_cv_enable_optbswap="$enableval"
], [ac_cv_enable_optbswap="yes"])
if test "$ac_cv_enable_optbswap" = yes; then
  AC_DEFINE(ENABLE_OPTBSWAP)
fi
])
